...
class WP_Advancedcheckout_OnepageController extends Mage_Checkout_OnepageController
{
    ...
    {       
        if ($data = $this->getRequest()->getPost())
        {
            ...
            Mage::helper('advancedcheckout')->setLocation($data['checkout_method']);
            ...
            Mage::helper('advancedcheckout')->setFormData( $data );
            # Set up method
            if (Mage::getSingleton('customer/session')->isLoggedIn())
            {
            ...
            else
            {
                ...
                Mage::helper('advancedcheckout')->setFormData( $data );
                Mage::helper('advancedcheckout')->setHasEmail( $data['email'] ? true : false );
            ...
            $locate = Mage::helper('advancedcheckout')->getLocation();
            $subtotal = $this->getOnepage()->getQuote()->getSubtotal();
            $min_subtotal = Mage::helper('advancedcheckout')->confDefaultMinPrice();
            $express_tax = Mage::helper('advancedcheckout')->confExpressPrice();
            $local_tax = Mage::helper('advancedcheckout')->confTax();
            $tax = 0;
            
            if ($locate === WP_Advancedcheckout_Block_Oneclick_Form::TYPE_OTHER)
            {
            ...
            if ($orderId = $this->getOnepage()->getCheckout()->getLastOrderId()){
                ...
                $payment = Mage::getModel('advancedcheckout/payment')->load($orderId, 'order_id');
                if (!$payment->getId()){
...
//            print_r($result);
    ...
    protected function _getArrayVal($data = array(), $key = 'none')
    {
        ...
        elseif($key === 'city')
        {
            return Mage::helper('advancedcheckout')->__('Москва');
        }
